...
평가 세트·지표·실측 결과는 **[EVALUATION.md](EVALUATION.md)** 참고. 재현:

```bash
python benchmark.py         # M1 판별력 · M2 최적성/탐색효율 · M3 독성 가드레일 (로컬, 무료)
python benchmark_agent.py   # M4 에이전트 효율 · M5 심사 효과 (LLM 필요)
```

핵심 실측: 판별력 **AUC 0.807**(양성 vs 무작위) · 라이브러리 최적 **BBB 0.960**(직접융합+SynB1 — 무링커가 최적일 수 있음을 포착) ·
독성 검출 **2/2**(멜리틴·마스토파란 탈락). deepB3P의 양이온성 CPP 편향 등 한계도 정직하게 기록.
